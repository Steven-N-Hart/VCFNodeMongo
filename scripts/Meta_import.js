...
if (lines.length === 1) {
    console.log('meta file has only 1 line');
    process.exit(2);
}
...
if (sampleIndex < 0 || studyIndex < 0 || kitIndex < 0) {
    ...
    console.log('sampleIndex=' + sampleIndex + '  studyIndex=' + studyIndex + '  kitIndex=' + kitIndex);
    process.exit(2);
}
...
MongoClient.connect(url, function (err, db) {
    ...
    //db.close();
    setTimeout(function(){db.close()}, 2000); ///// THIS IS REALLY HACKY!!!! Need to fix callback cascade.
});
